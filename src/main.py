...
def speed_func(t, freq, t1, t2):
    ...
    if t1 / sec_in_msec <= t <= (t1 + 1 / freq) / sec_in_msec:
        ...
        0 <= t < t1 / sec_in_msec
        or (t1 + 1 / freq) / sec_in_msec
        < t
        <= (t1 + 1 / freq + t2) / sec_in_msec
    ):
        return 0
    else:
        raise ValueError(
            f"`t` must be between 0 and {(t1 + 1 / freq + t2) / sec_in_msec}"
        )
...
font = pg.font.SysFont(
    name=conf["font_name"], size=conf["font_size"], bold=True
)
...
for id, char in enumerate(conf["alphabet"]):
    ...
    cells[char] = {
        ...
        "t1": set_t(conf["delay_t1_scale"], conf["is_random_delay"]),
        "t2": set_t(conf["delay_t2_scale"], conf["is_random_delay"]),
        "start_t": 0,
        "moving": False,
        "prev_speed": 0,
    }
